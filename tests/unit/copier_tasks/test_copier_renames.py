...
_COVERAGERC_RENAME = {"from": ".config/.coveragerc", "to": "backend/.coveragerc"}
...
class TestWhenRenamesTaskRuns:
    def test_Given_target_dir_exists__Then_file_moved_and_reported(self, tmp_path: Path) -> None:
    ...
    def test_Given_target_dir_absent__Then_file_left_in_place(self, tmp_path: Path) -> None:
        template_dir = tmp_path / "template"
        template_dir.mkdir()
        _declare_renames(tmp_path, [_COVERAGERC_RENAME])
        dst_dir = tmp_path / "destination"
        ...
        assert not (dst_dir / "backend").exists()
