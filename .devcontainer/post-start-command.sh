...
if ! bd ready; then
    ...
    rm -rf .claude/.beads && bd init --server-host=$BEADS_DOLT_SERVER_HOST --skip-hooks --stealth --prefix=work </dev/null && git -c core.hooksPath=/dev/null restore --source=HEAD --staged --worktree .claude/.beads
fi
